...
/* ThumbySNES: render one BG layer's full 256-pixel line into the
 * per-layer pixel cache. Splits hi-priority / lo-priority pixels into
 * separate buffers so ppu_getPixel can jump straight to the matching
 * priority slot.
 *
 * Only safe for the simple case — no mosaic, no Mode 7, no hires mode
 * 5/6, no OPT mode 2/4/6. ppu_runLine guards the call with the same
 * eligibility check; ppu_getPixel re-checks `bgLineCacheValid` before
 * sampling. */
LAKESNES_HOT static void ppu_renderBgLine(Ppu* ppu, int layer, int line) {
  ...
  // Walk output x in 8-pixel tile-aligned steps. There can be up to
  // 33 tiles spanning the visible 256 pixels when hScroll is misaligned.
  int outX = -(hScroll & 0x7);  // first tile may start before x=0
  int srcX = hScroll & ~0x7;    // tile boundary in source coords
  while (outX < 256) {
    // Tilemap address for this tile column, including X-wide selection.
...
LAKESNES_HOT void ppu_runLine(Ppu* ppu, int line) {
  ...
  /* ThumbySNES: hand the just-rendered scanline to the frontend so it
   * can downscale + blit before the next line stomps the buffer. */
  if(ppu->scanlineCb) ppu->scanlineCb(ppu->scanlineUser, line, ppu->pixelBuffer);
}
...
static void ppu_handlePixel(Ppu* ppu, int x, int y) {
  ...
  if(!ppu->forcedBlank) {
    ...
    bool mathEnabled = !ppu->skipColorMath && mainLayer < 6 && ppu->mathEnabled[mainLayer] && !(
  ...
  /* ThumbySNES single-line buffer: drop the row offset entirely.
   * scanlineCb in ppu_runLine flushes after each line. */
  ppu->pixelBuffer[x * 8 + 0 + ppu->pixelOutputFormat] = ((b2 << 3) | (b2 >> 2)) * ppu->brightness / 15;
  ppu->pixelBuffer[x * 8 + 1 + ppu->pixelOutputFormat] = ((g2 << 3) | (g2 >> 2)) * ppu->brightness / 15;
  ppu->pixelBuffer[x * 8 + 2 + ppu->pixelOutputFormat] = ((r2 << 3) | (r2 >> 2)) * ppu->brightness / 15;
  ppu->pixelBuffer[x * 8 + 4 + ppu->pixelOutputFormat] = ((b << 3) | (b >> 2)) * ppu->brightness / 15;
  ppu->pixelBuffer[x * 8 + 5 + ppu->pixelOutputFormat] = ((g << 3) | (g >> 2)) * ppu->brightness / 15;
  ppu->pixelBuffer[x * 8 + 6 + ppu->pixelOutputFormat] = ((r << 3) | (r >> 2)) * ppu->brightness / 15;
}
...
static int ppu_getPixel(Ppu* ppu, int x, int y, bool sub, int* r, int* g, int* b) {
  ...
  for(int i = 0; i < layerCountPerMode[actMode]; i++) {
    ...
    if(layerActive) {
      if(curLayer < 4) {
        /* ThumbySNES fast path: when ppu_runLine pre-rendered this
         * BG layer for the line, just sample the cache. */
        if (ppu->bgLineCacheValid[curLayer]) {
          pixel = ppu->bgLine[curLayer][curPriority ? 1 : 0][x];
        } else {
        // bg layer
        int lx = x;
        int ly = y;
...
void ppu_putPixels(Ppu* ppu, uint8_t* pixels) {
  /* ThumbySNES: legacy stub. With per-line scanlineCb the full-frame
   * pixelBuffer no longer exists — frontends must use the callback
   * (see ppu_setScanlineCallback). Kept for ABI compat with code that
   * still calls snes_setPixels. */
  (void)ppu; (void)pixels;
  return;
  /* unreachable — see comment above on per-line callback architecture. */
}
