...
/* ThumbySNES XIP toggle — when nonzero, snes_loadRom will (a) reject
 * headered ROMs (no in-place memmove possible against flash), and
 * (b) call cart_load with the caller's pointer rather than allocating
 * a power-of-2-expanded heap copy. Set via snes_setRomMode below. */
...
bool snes_loadRom(Snes* snes, const uint8_t* data, int length) {
  ...
  /* ThumbySNES XIP path: skip the heap copy + power-of-2 mirror
   * expansion. cart_load will retain the caller's pointer.
   *
   * Trade-off: cart_read* index with `& (romSize - 1)` which silently
   * mis-addresses on non-power-of-2 ROMs (3 MB titles like Super
   * Metroid, FF3). Those games may glitch in upper banks. We accept
   * that for first device boot — non-power-of-2 fix lands later. */
  int newLength;
  uint8_t* newData;
  bool xipPath = (thumbysnes_cart_xip != 0);
  if(xipPath) {
    newLength = length;
    newData   = (uint8_t*)data;   /* not actually written */
  } else {
